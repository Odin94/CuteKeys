...
export const DashboardPage = () => {
  ...
  const totalAttempts = allHotkeys.reduce(
    (s, h) => s + (stats.hotkeyPerformance[h.id]?.totalAttempts ?? 0),
    0,
  );
  const totalCorrect = allHotkeys.reduce(
    (s, h) => s + (stats.hotkeyPerformance[h.id]?.correctAttempts ?? 0),
    0,
  );
    ...
    <PageWrapper>
      <div className="max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          >
            <p className="font-display font-bold text-2xl text-[#3E2723] dark:text-[#F8F8F2]">
              {totalAttempts}
            </p>
          ...
          <motion.div
            ...
            className="bg-white dark:bg-[#3A3550] rounded-xl border border-[#F5E6D8] dark:border-[#5A5570] p-4 text-center"
          >
            <p className="font-display font-bold text-2xl text-[#F43F5E] dark:text-[#FFB8D1]">
              {weakHotkeys.length}
            </p>
        ...
        {app.sets.map((set) => (
          <motion.div
            ...
            className="mb-6"
          >
            <h3 className="font-display font-bold text-[#3E2723] dark:text-[#F8F8F2] mb-3">
              {set.name}
            </h3>
            <HotkeyStatsTable
              hotkeys={set.hotkeys}
