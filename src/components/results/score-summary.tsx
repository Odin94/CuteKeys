...
export const ScoreSummary = ({ session }: ScoreSummaryProps) => {
  ...
  const avgTime =
    session.attempts
      ...
      .reduce((s, a) => s + (a.responseTimeMs ?? 0), 0) /
  ...
  return (
    <div className="text-center">
      ...
      <div className="grid grid-cols-3 gap-4">
        <Stat
          icon={<Target className="h-5 w-5 text-[#8B5CF6] dark:text-[#C5A3FF]" />}
          label="Accuracy"
          value={`${Math.round(accuracy * 100)}%`}
        />
        <Stat
          icon={<Zap className="h-5 w-5 text-[#FB923C]" />}
          label="Avg. Speed"
          value={`${(avgTime / 1000).toFixed(1)}s`}
        />
        <Stat
          icon={<Flame className="h-5 w-5 text-[#F43F5E] dark:text-[#FFB8D1]" />}
          label="Best Streak"
          value={`${session.bestStreak}x`}
        />
      </div>
    </div>
  );
