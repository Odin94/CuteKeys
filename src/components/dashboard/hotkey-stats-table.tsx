...
export const HotkeyStatsTable = ({ hotkeys, performance, onToggleExclude }: HotkeyStatsTableProps) => (
        ...
        >
          ...
          <div className="text-right text-sm min-w-[80px]">
            {perf && perf.totalAttempts > 0 ? (
              <>
                ...
                <p className="text-xs text-[#8D6E63] dark:text-[#B0BEC5]">{perf.totalAttempts} tries</p>
              </>
            ) : (
          ...
          {perf && perf.avgResponseTimeMs > 0 && (
            <div className="text-right text-sm min-w-[50px]">
              <p className="text-xs text-[#8D6E63] dark:text-[#B0BEC5]">{(perf.avgResponseTimeMs / 1000).toFixed(1)}s</p>
            </div>
          )}
